...
當你完成基礎建置，或是**自定義替換 LLM 核心（如 Ollama、Groq）與 Memory 組件（如 Redis）**後，可以使用以下測試矩陣來驗證整條 Pipeline 的語意理解與路由防禦是否完全導通：

| 測試情境 | 測試輸入 (Input) | 預期行為 / 驗證目的 | 備註（自定義模型調整重點） |
| :--- | :--- | :--- | :--- |
| **基礎路由** | 傳送任意照片 | 驗證 Webhook、n8n 多媒體節點、以及 Vision 模型能力是否暢通。 | 若更換本地模型，需確認該模型是否支援多模態（Vision）。 |
| **防錯機制** | 傳送 LINE 貼圖 | 驗證系統能正確識別 `Sticker` 型態，給予罐頭回應或安全忽略，不觸發 LLM。 | 避免貼圖導致工作流報錯中斷。 |
| **核心意圖識別** | 「我肚子好痛喔」 | 驗證 LLM 能精準命中「健康照護/需要關懷」意圖，並觸發溫馨回應。 | 這是最直覺的強意圖案例，多數模型皆能正確識別。 |
| **模糊邊界測試** | 「早上起來一直流鼻水」 | 驗證 LLM 對於較生活化、非緊急症狀的感知能力。系統應將其歸類為健康關懷而非日常閒聊。 | **地雷案例**：中小型本地模型（如 Ollama 執行 Llama3/Gemma）常將此誤判為一般閒聊。若無回應，請優化意圖分類的 Prompt。 |
| **雜訊過濾機制** | 「今天天氣真好」 | 系統應**不回應**或僅做記錄。驗證日常閒聊不會被誤判為關懷事件。 | 測試防洗版邊界，確保 Bot 不會在群組內過度打擾。 |
| **記憶連續性** | 1. 「我今天感冒請假」<br>2.（隔五分鐘）「現在好多了」 | 1. 系統回應感冒關懷。<br>2. 系統能聯結上下文，回應如：「那就好，還是要多休息！」 | 驗證 Memory（如 Redis）有正確帶入 Prompt，且模型能讀懂歷史紀錄。 |

> 💡 **進階排查提示**：如果你更換了本地模型 (Ollama) 後發現「流鼻水」等案例沒有回應，通常是因為模型輸出的格式（例如 JSON 欄位）跑掉，或是被誤判為閒聊分支。建議打開 n8n 的 `Execution History` 點開該次事件，查看 LLM 節點的原始 Output 進行 Prompt 微調。
